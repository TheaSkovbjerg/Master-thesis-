...
client = AsyncClient(api_key='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')
...
EXCLUDED_PERSONA_BOOLEANS = set().union(*PROGRAM_FIELDS.values()) | {
    "boolean_classify_similar_relevant_program",
    "boolean_classify_evidence_of_grade_point_average",
    "boolean_classify_master_grade_point_average",
}
...
def build_scorecard(npa, persona_score):
    scorecard = {
        ...
        "gender_score": score_gender(npa.gender),
    }
...
async def send_input():
    ...
    try:
        ...
        candidate_task = client.beta.chat.completions.parse(
            ...
            messages=[
                {"role": "system", "content": (
                    ...
                    "Extract gender only if it is explicitly stated or unambiguous from pronouns in the text; otherwise return null. "
        ...
        winning_persona_class, winning_persona, highest_score, _, _, _ = max(
            persona_results,
